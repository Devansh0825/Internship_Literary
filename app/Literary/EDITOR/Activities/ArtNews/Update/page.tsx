...
interface News {
 ...
 function UpdateNewsinner() {
  ...
  useEffect(() => {
    const fetchNews = async () => {
      ...
      try {
        ...
        const response = await fetch(`${process.env.NEXT_PUBLIC_API_URL}/literaryartNews/${newsId}`)
        if (!response.ok) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const response = await fetch(`${process.env.NEXT_PUBLIC_API_URL}/literaryclub-news-update/${newsId}`, {
        method: "PUT",
        headers: {
      ...
      router.push(`/Literary/EDITOR/Activities/literaryArtNews`)
    } catch (error) {
      // toast({
      //   title: "Error",
