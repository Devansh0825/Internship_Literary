...
const ClubNewsForm = () => {
  ...
  const handleClickSubmitId = (id: any) => {
    router.push(`/Literary/EDITOR/Activities/ArtNews/ParticularArtNews?id=${id}`);
  };

  
  const gotoUpdate = (id: any) => {
    router.push(`/Literary/EDITOR/Activities/ArtNews/Update?id=${id}`);
  }
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const res = await fetch(`${process.env.NEXT_PUBLIC_API_URL}/literaryclubnews`, {
        method: "POST",
  ...
  const fetchNews = async () => {
    setLoading(true);
    try {
      const res = await fetch(`${process.env.NEXT_PUBLIC_API_URL}/literaryclubnewsviewallpost`);
      const data = await res.json();
