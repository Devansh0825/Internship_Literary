...
function ArtClubParticularJournalInner() {
  ...
  useEffect(() => {
    const fetchJournal = async () => {
      try {
        const response = await fetch(`${process.env.NEXT_PUBLIC_API_URL}/literaryartJournal/${id}`)
        if (!response.ok) {
...
export default function ArtClubParticularJournalPage() {
  return (
    <Suspense fallback={<div className="min-h-screen flex items-center justify-center"><div className="animate-spin rounded-full h-8 w-8 border-b-2 border-primary"></div></div>}>
      <ArtClubParticularJournalInner />
    </Suspense>
  );
}
