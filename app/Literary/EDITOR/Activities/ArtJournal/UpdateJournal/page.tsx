...
function UpdateJournalinner() {
  ...
  useEffect(() => {
    const fetchJournal = async () => {
      ...
      try {
        ...
        const response = await fetch(`${process.env.NEXT_PUBLIC_API_URL}/literaryartJournal/${journalId}`)
        if (!response.ok) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const response = await fetch(`${process.env.NEXT_PUBLIC_API_URL}/literaryclub-journal-update/${journalId}`, {
        method: "PUT",
        headers: {
